...
- **Identified patient at handover (0.7.3, opt-in):** `missione.paziente` puts the patient's local MPI identifier, name,
  gender and birth date into the CH EMS document that the receiving hospital asked for — into that document only;
  EPR-SPID and AHVN13 are refused. Whoever produces that document handles a health record: the same duties as for the
  paper protocol. The fallback ledger keeps digests (tested), the board is not involved (by construction: the document is
  built by the library, not by a server endpoint).
