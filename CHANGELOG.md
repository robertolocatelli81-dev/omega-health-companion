...
- **Round 2 (Opus 5) under the default, all measured and fixed:** the incident summary (`/incidenti`, `/incidente/<id>`)
  ...
  the encrypted journal file; a 2-byte sequence appears by chance in random ciphertext (1 red in 30 runs, measured) — the memory-only markers are now 18 to 33 bytes (0 red in 40 runs); the actual encryption control derives three persisted markers from the store's own compact serializer (`bacheca_store.serializza`), asserts each is present in the plaintext the store encrypts and absent from the raw file (round 10: a hand-written marker with spaces could never have matched compact JSON — a null control, found by Opus 5 and replaced; positive control: a plaintext sibling row written by the store turns the test red on the derived marker `"stato":"saturo"`). The encryption was never at fault.
